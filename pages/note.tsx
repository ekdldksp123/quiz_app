...
const Note: NextPage = () => {
  ...
  const nextHandler = (e: Event) => {
    if (pageIndex + 1 <= notes.length) {
      ...
      page.parentElement.style.transform = `translateX(calc(-100 * ${
        (pageIndex + 1) % notes.length
      }%))`;
      if (pageIndex + 1 < notes.length) setPageIndex(pageIndex + 1);
      else setPageIndex(0);
    }
  };
  ...
  const prevHandler = (e: Event) => {
    ...
    if (pageIndex - 1 >= 0) {
      const page = document.querySelectorAll(".page")[pageIndex];
      page.parentElement.style.transform = `translateX(calc(${
        -100 * pageIndex + 100
      }%))`;
      if (pageIndex < 0) setPageIndex(notes.length - 1);
  ...
  useEffect(() => {
    if (window.localStorage.hasOwnProperty("notes")) {
      const wrongList: QuizContent[] = JSON.parse(
        window.localStorage.getItem("notes")
      );

      setNotes(
        wrongList.map((v: QuizContent, i: number) => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (next) next.removeEventListener("click", nextHandler);
      if (prev) prev.removeEventListener("click", prevHandler);
    };
  }, [notes, pageIndex]);
      ...
      <Slide>
        {notes.length > 0 &&
          notes.map((v: QuizContent, i: number) => {
            return (
              <WrongNote key={i} quiz={v} index={i} amount={notes.length} />
            );
          })}
      </Slide>
      <ArrowBtn id="prev">❮</ArrowBtn>
      <ArrowBtn id="next" css={next}>
        ❯
      </ArrowBtn>
    </Container>
  );
